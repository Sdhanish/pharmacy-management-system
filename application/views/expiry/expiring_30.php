...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-2xl font-bold text-slate-900 mb-0">Expiring Within 30 Days</h2>
            <span class="badge bg-amber-100 text-amber-800 font-semibold px-2.5 py-1 rounded-full text-xs">
                <?php echo number_format($total_rows ?? 0); ?> Watchlist Items
            </span>
        </div>
        <p class="text-xs sm:text-sm text-slate-500 mb-0">Inventory items reaching expiration within the next month. Prioritize for First-In-First-Out (FIFO) dispensary dispensing.</p>
    </div>
    <div class="flex items-center gap-2.5">
        <a href="<?php echo base_url('expiry/expiring-7-days'); ?>" class="btn btn-orange text-xs sm:text-sm font-semibold rounded-xl px-4 py-2.5 flex items-center gap-2 shadow-xs bg-orange-600 text-white hover:bg-orange-700 text-decoration-none">
            <i class="fa-solid fa-hourglass-start"></i>
            <span>View 7-Day Urgent (<?php echo $kpis['expiring_7_days'] ?? 0; ?>)</span>
        </a>
...
<!-- INFO BANNER -->
<div class="alert alert-warning rounded-2xl border-amber-200 bg-amber-50 text-amber-900 p-4 mb-6 text-xs sm:text-sm flex items-start gap-3">
    <i class="fa-solid fa-clock-rotate-left text-amber-600 text-lg mt-0.5 shrink-0"></i>
    <div>
        <h5 class="font-bold mb-1 text-amber-900 text-xs sm:text-sm">30-Day Dispensing Priority Advisory</h5>
        <p class="mb-0 text-xs text-amber-800">
            Apply FIFO (First In, First Out) rules. Ensure pharmacist dispensary counters prioritize dispensing these lots before newer shipments.
        </p>
    </div>
</div>
...
<div class="app-card p-4 sm:p-5 mb-6">
    <form action="<?php echo base_url('expiry/expiring-30-days'); ?>" method="GET" class="grid grid-cols-1 sm:grid-cols-12 gap-3.5 items-end">
        <div class="sm:col-span-10">
            <label for="search" class="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1.5">
                Search 30-Day Watchlist
            </label>
            <div class="relative">
                ...
                <input type="text" 
                       ...
                       class="form-control form-control-sm pl-9 text-xs rounded-xl border-slate-200 focus:border-amber-500 focus:ring-amber-500" 
                       placeholder="Search by medicine name, therapeutic category, supplier...">
            </div>
        </div>

        <div class="sm:col-span-2 flex items-center gap-2">
            <button type="submit" class="btn btn-warning btn-sm rounded-xl px-3.5 py-2 w-full text-xs font-semibold flex items-center justify-center gap-1.5 shadow-xs text-slate-900">
                <i class="fa-solid fa-filter text-[10px]"></i>
                <span>Filter</span>
            </button>
            <?php if (!empty($search)): ?>
                <a href="<?php echo base_url('expiry/expiring-30-days'); ?>" class="btn btn-light btn-sm rounded-xl px-3 py-2 text-xs font-semibold border border-slate-200 text-slate-600 hover:bg-slate-100 flex items-center justify-center text-decoration-none" title="Reset Search">
        ...
        <table class="table table-hover align-middle mb-0 text-xs sm:text-sm">
            <thead class="table-light text-[11px] text-slate-500 font-bold uppercase tracking-wider border-b border-slate-200">
                <tr>
                    ...
                    <th class="py-3.5 text-center">Current Units</th>
                    <th class="py-3.5">Expiration Date</th>
                    <th class="py-3.5 text-center">Time Remaining</th>
                    <th class="py-3.5 text-end">Value At Risk</th>
            ...
            <tbody class="divide-y divide-slate-100">
                <?php if (isset($medicines) && !empty($medicines)): ?>
                    <?php foreach ($medicines as $med): ?>
                        ...
                        $is_7_days = ($days_left <= 7);
                        $stock = (int)($med['stock_quantity'] ?? 0);
                        $val = (float)($med['total_at_risk_value'] ?? ($stock * (float)$med['price']));
                        ?>
                        <tr class="hover:bg-amber-50/40 transition-colors <?php echo $is_7_days ? 'bg-orange-50/20' : ''; ?>">
                            ...
                            <td class="py-3.5 font-mono text-xs font-bold <?php echo $is_7_days ? 'text-orange-600' : 'text-amber-700'; ?>">
                            ...
                            <td class="py-3.5 text-center">
                                <?php if ($is_7_days): ?>
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-bold bg-orange-100 text-orange-800 border border-orange-200 animate-pulse">
                                        ...
                                        <span><?php echo $days_left; ?> days left (Urgent)</span>
                                    </span>
                                <?php else: ?>
                            ...
                            <td class="py-3.5 text-end font-mono font-bold text-xs text-amber-800">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="8" class="py-12 text-center text-slate-400">
                            ...
                            <p class="text-sm font-semibold text-slate-700 mb-1">No Medicines Expiring in 30 Days</p>
                            <p class="text-xs text-slate-400 mb-0">
                                <?php echo !empty($search) ? 'No items matching "' . html_escape($search) . '".' : 'All active stock is well beyond the 30-day expiration window.'; ?>
                            </p>
                        </td>
                    </tr>
                <?php endif; ?>
    ...
    <?php if (isset($pagination_links) && !empty($pagination_links)): ?>
        <div class="px-5 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50">
            <p class="text-xs text-slate-500 mb-0 font-medium">
                Showing <?php echo min($total_rows, ($offset ?? 0) + 1); ?> to <?php echo min($total_rows, ($offset ?? 0) + count($medicines)); ?> of <?php echo $total_rows; ?> expiring items
            </p>
            <div>
