...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-2xl font-bold text-slate-900 mb-0">
                Edit Stock Purchase #PO-<?php echo str_pad($purchase->id, 5, '0', STR_PAD_LEFT); ?>
            </h2>
        </div>
        <p class="text-xs sm:text-sm text-slate-500 mb-0">Modifying quantity will automatically rebalance medicine inventory stock.</p>
    </div>
    <div class="flex items-center gap-2.5">
        ...
        <button type="submit" form="editStockPurchaseForm" class="btn btn-emerald text-xs sm:text-sm font-semibold rounded-xl px-5 py-2.5 shadow-md shadow-emerald-600/20 hover:shadow-lg transition flex items-center gap-2">
            <i class="fa-solid fa-floppy-disk"></i>
            <span>Update & Rebalance Stock</span>
        </button>
    </div>
</div>
...
<!-- EDIT FORM -->
<form action="<?php echo base_url('stock/update/' . $purchase->id); ?>" method="POST" id="editStockPurchaseForm" class="needs-validation" novalidate>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        
        <!-- LEFT 2 COLS: PURCHASE ATTRIBUTES -->
            ...
            <div class="app-card p-5 sm:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="sm:col-span-2">
                        <label for="medicine_id" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                            Medicine Item <span class="text-rose-500">*</span>
                        </label>
                        <select name="medicine_id" id="medicine_id" class="form-select text-sm rounded-xl border-slate-200 focus:border-emerald-500 <?php echo form_error('medicine_id') ? 'is-invalid' : ''; ?>" required>
                            <option value="">-- Choose Medicine --</option>
                            <?php if (isset($medicines) && !empty($medicines)): ?>
                                <?php foreach ($medicines as $m): ?>
                                    <option value="<?php echo $m['id']; ?>" <?php echo (set_value('medicine_id', $purchase->medicine_id) == $m['id']) ? 'selected' : ''; ?>>
                        ...
                        <select name="supplier_id" id="supplier_id" class="form-select text-sm rounded-xl border-slate-200 focus:border-emerald-500 <?php echo form_error('supplier_id') ? 'is-invalid' : ''; ?>" required>
                            ...
                            <?php if (isset($suppliers) && !empty($suppliers)): ?>
                                <?php foreach ($suppliers as $sup): ?>
                                    <option value="<?php echo $sup['id']; ?>" <?php echo (set_value('supplier_id', $purchase->supplier_id) == $sup['id']) ? 'selected' : ''; ?>>
                    ...
                    <div>
                        ...
                        <input type="date" 
                               name="purchase_date" 
                               id="purchase_date" 
                               value="<?php echo set_value('purchase_date', $purchase->purchase_date); ?>" 
                    ...
                    <div>
                        <label for="quantity" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
                            Purchased Quantity <span class="text-rose-500">*</span>
                        </label>
                        <input type="number" 
                               min="1" 
                               name="quantity" 
                               id="quantity" 
                               value="<?php echo set_value('quantity', $purchase->quantity); ?>" 
                    ...
                    <div>
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" 
                                   ...
                                   name="purchase_price" 
                                   id="purchase_price" 
                                   value="<?php echo set_value('purchase_price', $purchase->purchase_price); ?>" 
                ...
                <div>
                    ...
                    <textarea name="notes" 
                              id="notes" 
                              rows="3" 
                              class="form-control text-sm rounded-xl border-slate-200 focus:border-emerald-500"><?php echo set_value('notes', $purchase->notes); ?></textarea>
                </div>
            </div>
            ...
            <div class="app-card p-5 sm:p-6">
                ...
                <div class="space-y-3 text-xs mb-4">
                    <div class="flex justify-between py-1 border-b border-slate-100">
                        ...
                        <span class="font-mono font-bold text-slate-800" id="summaryQty"><?php echo $purchase->quantity; ?> units</span>
                    </div>
                    <div class="flex justify-between py-1 border-b border-slate-100">
                        <span class="text-slate-500">Unit Cost:</span>
                        <span class="font-mono font-bold text-slate-800" id="summaryPrice">$<?php echo number_format($purchase->purchase_price, 2); ?></span>
                    </div>
                    <div class="flex justify-between py-2 border-t border-slate-200 text-sm">
                        <span class="font-bold text-slate-800">Total Cost:</span>
                        <span class="font-mono font-extrabold text-emerald-700 text-base" id="summaryTotal">$<?php echo number_format($purchase->total_amount, 2); ?></span>
                    </div>
                </div>
                ...
                <div class="p-3 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-xs">
                    <div class="flex items-start gap-2">
                        ...
                        <div>
                            <strong class="block mb-0.5">Inventory Auto-Rebalance</strong>
                            <span>Changing the purchase quantity will adjust the live medicine stock by the net difference.</span>
                        </div>
                    </div>
                </div>
            </div>
